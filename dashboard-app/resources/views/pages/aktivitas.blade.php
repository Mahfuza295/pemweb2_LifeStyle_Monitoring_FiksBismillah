    ...
    <div class="activity-page activity-mobile-wrap max-w-5xl mx-auto">
        ...
        <section id="dailyView">
            ...
            <form id="dailySubmitForm" method="POST" action="{{ route('aktivitas.store') }}">
                ...
                {{-- Menyesuaikan dengan penamaan variabel request di Controller Anda --}}
                <<input type="hidden" name="makan" id="formMakan">
                    <input type="hidden" name="air_minum" id="formAirMinum">
                    <input type="hidden" name="olahraga" id="formOlahraga">
                    <input type="hidden" name="tidur" id="formTidur">
        ...
        <section id="sessionView" class="hidden">
            <div class="bg-white rounded-2xl border border-slate-200 shadow-sm p-5 sm:p-6">
                ...
                <form id="sessionForm" class="space-y-6">
                    ...
                    <div>
                        <label class="block text-sm font-bold text-slate-700 mb-2">
                            Jumlah Air Minum (Gelas)
                        </label>

                        <input type="number" min="0" max="30" step="1" id="gelas" placeholder="Contoh: 2"
                    ...
                    <div>
                        ...
                        <input type="number" min="0" max="300" step="1" id="olahragaDurasi" placeholder="Durasi dalam menit"
                            class="activity-field">
                    </div>
                    ...
                    <div id="tidurInputWrap" class="hidden">
                        ...
                        <input type="number" min="0" max="24" step="0.5" id="tidurJam" placeholder="Durasi tidur dalam jam"
                            class="activity-field">
                    </div>
                    ...
                    @foreach($riwayat as $item)
                        <div class="activity-card bg-white rounded-2xl p-5 sm:p-6">
                            ...
                            <div class="border-t pt-4 grid grid-cols-1 sm:grid-cols-2 gap-3 text-sm text-slate-600">
                                <p>
                                    <span class="font-semibold text-slate-700">Makanan:</span>
                                    {{ $item->makanan }}
                                </p>

                                <p>
                                    <span class="font-semibold text-slate-700">Minuman:</span>
                                    {{ $item->minuman }}
                                </p>

                                <p>
                                    <span class="font-semibold text-slate-700">Total Air Diminum:</span>
                                    {{ $item->jumlah_air }} Gelas
                                </p>

                                <p>
                                    <span class="font-semibold text-slate-700">Olahraga:</span>
                                    {{ $item->olahraga }} ({{ $item->durasi_olahraga }} Menit)
                                </p>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            tidurCheck.addEventListener('change', () => {
                tidurInputWrap.classList.toggle('hidden', !tidurCheck.checked);
                if (!tidurCheck.checked) tidurJam.value = '';
            });
            ...
            document.getElementById('btnKirimData').addEventListener('click', () => {
                ...
                // Gabungkan teks makanan & minuman dari semua sesi yang diisi
                const listMakanan = filledSessions.map(([key, data]) => `${sessionReadable[key]}: ${data.makanan}`).join(', ');
                const listMinuman = filledSessions.map(([key, data]) => `${sessionReadable[key]}: ${data.minuman}`).join(', ');
                const listOlahraga = filledSessions.map(([key, data]) => `${sessionReadable[key]}: ${data.olahragaJenis}`).join(', ');

                // Akumulasi angka total harian
                const totalAir = filledSessions.reduce((sum, [, data]) => sum + Number(data.gelas || 0), 0);
                const totalOlahragaDurasi = filledSessions.reduce((sum, [, data]) => sum + Number(data.olahragaDurasi || 0), 0);
                const totalTidur = filledSessions.reduce((sum, [, data]) => sum + Number(data.tidurJam || 0), 0);

                // Masukkan data ter-gabung ke dalam Hidden Input Form
                document.getElementById('formMakanan').value = listMakanan;
                document.getElementById('formMinuman').value = listMinuman;
                document.getElementById('formJumlahAir').value = totalAir;
                document.getElementById('formOlahraga').value = listOlahraga;
                document.getElementById('formDurasiOlahraga').value = totalOlahragaDurasi;
