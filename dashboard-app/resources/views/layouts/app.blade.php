        ...
        <aside class="w-64 bg-white shadow-lg fixed h-full">
            ...
            <nav class="p-4 space-y-2">
                ...
                <a href="{{ route('dashboard') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition
                   {{ request()->routeIs('dashboard') ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-100' }}">
                    <i class="fa-solid fa-chart-pie w-5 text-center"></i>
                    <span>Dashboard</span>
                </a>

                {{-- 🔐 Sembunyikan menu Input Aktivitas khusus untuk Admin --}}
                @if(auth()->user()->role != 'admin')
                    <a href="{{ route('aktivitas') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition
                       {{ request()->routeIs('aktivitas') ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-100' }}">
                ...
                @if(auth()->user()->role == 'admin')
                    <a href="{{ route('admin.artikel.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition
                       {{ request()->routeIs('admin.artikel.*') ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-100' }}">
                ...
                @if(auth()->user()->role != 'admin')
                    <a href="{{ route('artikel.index') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition
                       {{ request()->routeIs('artikel.index') ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-100' }}">
                        <i class="fa-solid fa-newspaper w-5 text-center"></i>
                ...
                <a href="{{ route('profil') }}" class="flex items-center gap-3 px-4 py-3 rounded-xl transition
                   {{ request()->routeIs('profil') ? 'bg-blue-600 text-white shadow-md' : 'hover:bg-slate-100' }}">
